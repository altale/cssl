...
def run_swiglu(
    ...
    swiglu = SwiGLU_FFN(d_model, d_ff, device=in_features.device, dtype=in_features.dtype)
...
def run_multihead_self_attention(
    ...
    """
    mha = MultiHeadSelfAttention(d_model=d_model, num_heads=num_heads, theta=None, max_seq_len=in_features.shape[-2], device=in_features.device, dtype=in_features.dtype)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    mha = MultiHeadSelfAttention(d_model=d_model, num_heads=num_heads, theta=theta, max_seq_len=in_features.shape[-2], device=in_features.device, dtype=in_features.dtype)
    mha.q_proj.weight.data.copy_(q_proj_weight)
    mha.k_proj.weight.data.copy_(k_proj_weight)
    mha.v_proj.weight.data.copy_(v_proj_weight)
    mha.output_proj.weight.data.copy_(o_proj_weight)
...
def run_rope(
    ...
    """
    rope = RotaryPositionalEmbedding(theta, d_k, max_seq_len, device=in_query_or_key.device, dtype=in_query_or_key.dtype)
...
def run_transformer_block(
    ...
    """
    block = TransformerBlock(d_model=d_model, num_heads=num_heads, d_ff=d_ff, max_seq_len=max_seq_len, theta=theta, device=in_features.device, dtype=in_features.dtype)
